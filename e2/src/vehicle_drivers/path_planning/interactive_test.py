...
class InteractivePlanner:
    def __init__(self):
        ...
        start_x, start_y = wps_to_local_xy(self.olon, self.olat, self.olat, self.olon)
    ...
    def on_click(self, event):
        ...
        yaw_rad = car_heading_to_planner_yaw(yaw_deg)
    ...
    def plan_path(self):
        ...
        #     state.pos[0] = state.pos[0] + self.map.grid_top_left[0]
        #     state.pos[1] = self.map.grid_top_left[1] - state.pos[1]
        ...
        #     step = max(1, len(self.path) // 50)  # Ensure we get at least 30 points
        #     self.path = self.path[::step] + [self.path[-1]]
