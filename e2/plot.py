...
def plot_from_csv(planned_path_file, actual_path_file):
    ...
    with open(planned_path_file, 'r') as f:
        ...
        for row in reader:
            ...
            planned_path.append(tuple(map(float, row)))
    ...
    with open(actual_path_file, 'r') as f:
        ...
        for row in reader:
            if row[0] == 'actual':
                ...
                # actual_path.append(tuple(map(float, row[1:5])))
                if len(row) > 5:
...
planned_path_file = "hybrid_astar_path_original.csv"
actual_path_file = "vehicle_trajectory_latest.csv"
