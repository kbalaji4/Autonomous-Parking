...
from dp import calc_distance_heuristic
from rsp import reeds_shepp_path_planning as rs
from rsp import plot_arrow, calc_paths, pi_2_pi
...
def plot_test(path, start, goal):
    ...
    """
    # Create a cost map with height=40 (y) and width=80 (x)
    cost_map = np.zeros((15, 30))
    
    # Example obstacles (indices are relative to cost_map grid)
    cost_map[10:15, 10:15] = 1e6  # first obstacle
    cost_map[20:25, 5:10] = 1e6   # second obstacle

    grid_height, grid_width = cost_map.shape  # grid_height=40, grid_width=80
    
    # The physical extents: x from -40 to 40, y from -40 to 0.
    fig, ax = plt.subplots(figsize=(8, 8))
    cax = ax.imshow(cost_map.T, origin='lower', cmap='binary',
                    extent=[-30, 0, -30, -15])
    
    # Set gridlines at every 1 m
    ax.set_xticks(np.arange(-30, 1, 1), minor=True)
    ax.set_yticks(np.arange(-30, -14, 1), minor=True)
    ax.grid(which='minor', color='gray', linestyle='-', linewidth=0.5)
    
    # Extract x, y, yaw from the trajectory states
    xs = [p[0] for p in path]
    ys = [p[1] for p in path]
    yaws = [p[2] for p in path]
    
    # Plot the path and orientation arrows
...
def main():
    ...
    start = (start_x, start_y, np.deg2rad(start_yaw))
    ...
    goal_yaw = np.deg2rad(args.goal_yaw)
    goal = (goal_x, goal_y, goal_yaw)

    print("start : ", start)
    print("goal : ", goal)

    # Convert obstacles into Gazebo frame
    ox_gaz = np.array(ox) - offset_x
    oy_gaz = np.array(oy) - offset_y

    # if show_animation:
    #     plt.plot(ox_gaz, oy_gaz, ".k")
    #     plot_arrow(start[0] - offset_x, start[1] - offset_y, start[2], fc='g')
    #     plot_arrow(goal[0] - offset_x, goal[1] - offset_y, goal[2])
    #     plt.grid(True)
    #     plt.axis("equal")
    ...
    if path:
        ...
        local_path = [(x - offset_x, y - offset_y, yaw) for x, y, yaw in (path.x_list, path.y_list, path.yaw_list)]
