...
def main():
    ...
    if show_animation:
        ...
        rs.plot_arrow(start[0] - offset_x, start[1] - offset_y, start[2], fc='g')
        rs.plot_arrow(goal[0] - offset_x, goal[1] - offset_y, goal[2])
        plt.grid(True)
        plt.axis("equal")
    ...
    x = np.array(path.x_list) - offset_x
    y = np.array(path.y_list) - offset_y
    yaw = path.yaw_list
